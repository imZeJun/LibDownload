...
import static com.meitu.mobile.browser.lib.download.constant.Downloads.Impl.STATUS_INSUFFICIENT_SPACE_ERROR;
...
public class StorageUtils {
    ...
    private static void freeCacheStorage(long bytes) {
        ...
        Log.d(TAG, "Found " + files.size() + " downloads on cache");
        ...
        for (ConcreteFile file : files) {
            ...
            if (now - file.file.lastModified() < MIN_DELETE_AGE) {
                Log.d(TAG, "Skipping recently modified " + file.file);
            } else {
                final long len = file.file.length();
                Log.d(TAG, "Deleting " + file.file + " to reclaim " + len);
                bytes -= len;
                file.file.delete();
            }
        }
